...
function requestNotificationPermission() {
    ...
    if (window.location.protocol === 'file:') {
        alert("Note: Browsers block standard notifications on local file paths (file:///). The window.alert fallback will still work perfectly!");
    }
...
// Improved Notification Logic - Real-time Precision + Window Alert Fallback
function checkNotifications() {
    ...
    myEvents.forEach(event => {
        ...
        if (Math.abs(now - eventTime) < 2000 && !event.notified) {
            ...
            // 1. Try sending standard system notification if permission granted
            if ("Notification" in window && Notification.permission === "granted") {
                sendNotification(event);
            }
            
            // 2. USER REQUEST: Trigger the robust browser window alert box
            // Wrapped in a slight timeout to ensure it doesn't freeze the render loop thread
            setTimeout(() => {
                window.alert(`⏰ SyncSchedule Reminder:\n\nTask: ${event.title}\nScheduled Time: ${new Date(event.start).toLocaleString()}`);
            }, 50);
...
function initModal() {
    ...
    document.getElementById('confirmBtn').onclick = () => {
        if (deleteId) {
            // Explicit string conversion fixes type conflicts
...
// Task sidebar item list generator
